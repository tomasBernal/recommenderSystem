...
public abstract class ReaderOfFiles {
	...
	// This function extract the name of the movies with the id
		public static ArrayList<Double> getUserProfile(String fileName){
			...
			try {
				...
				if( line == null) {
			        ...
			        while ((line = reader.readLine()) != null) {
			    		...
			    		profile.add(Double.parseDouble(line));
	...
	//This function obtains the genders of the movies
	public static ArrayList<String> getGenres(List<LinkedList<String>> movies){
		
		ArrayList<String> genders = new ArrayList<>();
		
		for (List<String> movie : movies) {
			
			//We start from the item number 2 because it is the first gender of the film
			for(int i = 2; i < movie.size(); i++) {
				
				if(!genders.contains(movie.get(i))) {
					genders.add(movie.get(i));
				}
	...
	public static List<ArrayList<Double>> getGenresMatrix(List<LinkedList<String>> movies,List<String> genders){
		
		List<ArrayList<Double>> matrix = new LinkedList<>();
		
		//First I fill out the matrix with false value
		...
		for (int i = 0; i < movies.size();i++) {
			...
			for(int j = 2; j < movies.get(i).size(); j++) {
				...
				//We obtain the index of the film from the genders and we put this value to true because this film has this gender 
				matrix.get(i).set(genders.indexOf(movies.get(i).get(j)), 1.0);
		...
		// Creation of the reader
	  	...
	  	line = reader.readLine();
			...
			if(Pattern.matches("[12345]", parts[2]) || Pattern.matches("[1234]\\.[05]", parts[2]) || Pattern.matches("5\\.0", parts[2])) {
		        ...
		        {
		        	// ToySet
		            //file = new FileWriter("/Users/tomas/Desktop/AI/Implementacion/DataSets/MoviesDataset/toySet/Toy_Pre-procesed.txt");
		            // Big csv
